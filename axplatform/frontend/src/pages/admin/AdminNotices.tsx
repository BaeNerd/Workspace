      ...
      <div style={{ fontFamily: "var(--font-ui)", background: "#F8FAFC", minHeight: "100vh", color: "#0F172A" }}>
        ...
        <div style={{ display: "flex" }}>
          ...
          <main style={{ flex: 1, minWidth: 0, minHeight: "calc(100vh - 56px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 32 }}>
            <div style={{ maxWidth: 460, textAlign: "center", background: "#fff", border: "1.5px solid #E2E8F0", borderRadius: 12, padding: "36px 32px" }}>
              <div style={{ fontSize: 15, fontWeight: 800, color: "#0F172A", marginBottom: 8 }}>전사 관리자 전용 화면입니다</div>
    ...
    <div style={{ fontFamily: "var(--font-ui)", background: "#F8FAFC", minHeight: "100vh", color: "#0F172A" }}>
      ...
      <div style={{ display: "flex" }}>
        ...
        <main style={{ flex: 1, display: "flex", minWidth: 0, minHeight: "calc(100vh - 56px)" }}>
          ...
          {/* ===== 좌측: 소식 목록 ===== */}
          <div style={{ width: 320, flexShrink: 0, borderRight: "1px solid #E2E8F0", background: "#fff", display: "flex", flexDirection: "column" }}>
            <div style={{ padding: "16px 16px 12px", borderBottom: "1px solid #F1F5F9" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: "#0F172A" }}>소식 <span style={{ color: "#94A3B8", fontWeight: 500 }}>{notices.length}</span></span>
                ...
                return (
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                      ...
                      {!n.visible && (
                        <span style={{ fontSize: 9, fontWeight: 700, color: "#94A3B8", background: "#F1F5F9", padding: "1px 6px", borderRadius: 10, flexShrink: 0 }}>숨김</span>
                      )}
                    </div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: n.visible ? "#0F172A" : "#94A3B8", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginBottom: 2 }}>{n.title}</div>
                    <div style={{ fontSize: 10.5, color: "#94A3B8", fontFamily: "var(--font-mono)" }}>{n.date}</div>
                  </div>
                );
              })}
              {listItems.length === 0 && (
                <div style={{ padding: "30px 16px", textAlign: "center", fontSize: 12, color: "#94A3B8" }}>표시할 소식이 없습니다.</div>
              )}
            </div>
          </div>
          ...
          <div style={{ flex: 1, minWidth: 0, padding: "24px 32px", overflowY: "auto" }}>
            {!displayData ? (
              <div style={{ padding: 60, textAlign: "center", color: "#94A3B8", fontSize: 13 }}>좌측에서 소식을 선택하거나 새로 작성하세요.</div>
            ) : (
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, gap: 12 }}>
                  <div style={{ minWidth: 0, flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                      ...
                      {!displayData.visible && <span style={{ fontSize: 10, fontWeight: 700, color: "#94A3B8", background: "#F1F5F9", padding: "2px 8px", borderRadius: 12 }}>숨김</span>}
                    </div>
                    ...
                    <div style={{ fontSize: 11, color: "#94A3B8", fontFamily: "var(--font-mono)", marginTop: 3 }}>{displayData.id || "(신규)"} · {displayData.date}</div>
                  </div>
                ...
                <SectionBlock title="내용">
                  ...
                  <FieldRow label="제목">
                    {isEditing
                      ...
                      : <div style={{ fontSize: 14, fontWeight: 600, color: "#0F172A" }}>{displayData.title}</div>}
                  </FieldRow>
                  ...
                  <FieldRow label="본문" hint="목록에서 펼치면 표시되는 상세 내용">
                    {isEditing
                      ...
                      : <div style={{ fontSize: 13, color: "#475569", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{displayData.body}</div>}
                  </FieldRow>
                  ...
                  <FieldRow label="게시일" hint="YYYY.MM.DD">
                    {isEditing
                      ...
                      : <div style={{ fontSize: 13, color: "#334155", fontFamily: "var(--font-mono)" }}>{displayData.date}</div>}
                  </FieldRow>
                </SectionBlock>
...
const ToggleRow = ({ title, desc, on, onLabel, offLabel, onToggle }: {
  ...
  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
    <div style={{ minWidth: 0 }}>
      <div style={{ fontSize: 13, fontWeight: 600, color: "#0F172A", marginBottom: 3 }}>{title}</div>
    ...
    <button onClick={onToggle} style={{
      ...
      border: `1.5px solid ${on ? "#6EE7B7" : "#E2E8F0"}`,
