...
def_slide(
    ...
    sections=[
        ...
        ("준수 규칙", ["가시성·승인 자격 = ownerCompany 단일 축 · 전사 슬롯 admin 전용",
...
def_slide(
    ...
    sections=[
        ...
        ("준수 규칙", ["가시성·삭제 권한 = ownerCompany 축(검토와 동일 판정)",
                   "삭제는 확인+복구 불가 고지",
                   "직접 등록도 ID 규칙(0.3)"]),
...
def_slide(
    ...
    sections=[
        ...
        ("준수 규칙", ["프리셋 4종 고정(최근3·6개월/올해 전체/범위 지정 최대 24개월, 서버 재검증)",
...
def_slide(
    ...
    sections=[
        ...
        ("준수 규칙", ["고정 분류 삭제 시 기존 카드 공란 처리 가능 안내(참조 무결성은 서버 일관)",
                   "자유 태그 출처 7카테고리 대응",
                   "편입 조합 2종 한정"]),
