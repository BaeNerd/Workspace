# -*- coding: utf-8 -*-
"""
AX Platform 화면정의서 v2 공용 헬퍼 (PHASE 1·2 공유)
- 본문: 무언어 무채색 실루엣 와이어프레임 + 빨간 마커 → 우측 Description 일원화
- p0: 좌(구조 트리 + 사용자 흐름) / 우(정의·목적·룰·기획의도 톤다운)
...
def desc_panel(area, items, title="Description"):
...
def screen(prs, hdr, draw_fn, desc_items, active=None, page_no=1, page_total=1, subtitle=""):
    """hdr=(screen_name,title,screen_id,path). draw_fn(slide,inner,ctx)."""
    ...
    desc_panel(area, [it for it in desc_items if it[0] in act])
    return area
...
def def_slide(prs, screen_id, screen_name, tree, flow, sections, flow_branch=None,
              tree_caption="화면 구조", flow_caption="사용자 흐름"):
    s = add_slide(prs)
    # 타이틀 행
    ...
    flow_cap_y = max(tree_bottom + 0.12, top + 3.2)
    if flow_cap_y > CONTENT_BOTTOM - 1.2:
        flow_cap_y = CONTENT_BOTTOM - 1.2
