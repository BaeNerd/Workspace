...
def_slide(
    ...
    sections=[
        ...
        ("준수 규칙", ["관계사 귀속 미표시(ownerCompany는 API 제공·미렌더)",
...
def_slide(
    ...
    sections=[
        ...
        ("준수 규칙", ["노출 범위 입력 없음(신규=전사 공용, 0.9)",
                   "절감 시간 표준 직렬화(0.8)",
...
def_slide(
    ...
    sections=[
        ...
        ("준수 규칙", ["알림 이동은 카드 ID 접두어에서 파생한 경로",
                   "공유 모드에서는 벨 미노출"]),
...
def_slide(
    ...
    sections=[
        ...
        ("기획 의도", ["소개·가이드 통합으로 학습 동선 단일화",
                    ...
                    "부제는 확정 문안을 그대로 사용"]),
        ("준수 규칙", ["승인 안내는 0.7 정합 유지(정책 변경 시 동기화 대상)",
                   "정적 콘텐츠 — 개인화·권한 분기 없음"]),
