...
// ★ 신규 — PlatformItem 관계사 집계용 최소 타입 + 목업 데이터
// TODO: 실제 연동 시 GET /api/v1/admin/platform-items?fields=company 응답으로 교체
// (전체 PlatformItem을 다 불러올 필요 없이 company 필드만 집계용으로 조회하면 충분)
...
// 관계사 코드 → 해당 관계사를 대상으로 하는 PlatformItem 수 집계
// (company가 빈 배열인 "전사 공용" 항목은 모든 관계사에 영향을 주므로 전체 카운트에 포함)
...
// ★ 변경 — 체크박스 목록 각 행에 "PlatformItem N건" 카운트 배지 추가
    ...
    <div style={{ fontFamily: "'Inter', -apple-system, sans-serif", background: "#F8FAFC", minHeight: "100vh", color: "#0F172A" }}>
      ...
      <div style={{ display: "flex" }}>
        ...
        <main style={{ flex: 1, padding: "28px 32px", minWidth: 0 }}>
          ...
          {/* ===== 관계사 노출 관리 ===== */}
          <div style={{ background: "#fff", border: "1.5px solid #E2E8F0", borderRadius: 10, marginBottom: 20, padding: "18px 18px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 20, alignItems: "start" }}>
            <div>
              ...
              {/* ★ 신규 — 부서 목록 카운트는 Project 전용임을 안내 (PlatformItem은 부서 계층이 없어 매칭 불가) */}
              <div style={{ background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 8, padding: "8px 12px", marginBottom: 14, fontSize: 11, color: "#64748B" }}>
                아래 부서별 "프로젝트 N건" 카운트는 일반 프로젝트만 집계합니다. n8n·나만의 비서·AI Agent는 부서 단위가 아닌 관계사 단위로 집계되며, 위의 관계사별 노출 관리 영역에서 "플랫폼 N" 배지로 확인할 수 있습니다.
              </div>
                      ...
                      <div key={dept.id} style={{
                        ...
                        {isEditing ? (
                          <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1, flexWrap: "wrap" }}>
                            ...
                            />
                            <select value={editData.company !== undefined ? editData.company : dept.company} onChange={e => setEditData(p => ({ ...p, company: e.target.value }))}
